...
const APPROVED = {
  englishT01: {
    ...
    // Baseline v1.10.0 — see ENGLISH_BATCH1_BASELINE_APPROVAL.json's
  ...
  arabicT01: {
    ...
    // Baseline v1.0.11 — see ARABIC_BATCH1_BASELINE_APPROVAL.json's
...
describe("Batch 1 integrity — no unapproved topic or asset is loaded", () => {
  it("RAW_CONTENT_BY_TOPIC contains exactly the seven internally authorized topics", () => {
    const keys = Object.keys(RAW_CONTENT_BY_TOPIC).sort();
    expect(keys).toEqual([...APP_TOPIC_ORDER].sort());
  });

  it("no placeholder or out-of-scope topic ID (e.g. ch01-t06, ch01-t09) is present anywhere in the loaded set", () => {
    const outOfScope = ["ch01-t06", "ch01-t07", "ch01-t09", "ch01-t11", "ch01-t12", "ch01-t13", "ch01-t14"];
